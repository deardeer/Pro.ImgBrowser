...
class Hist1D:
    ...
    def _computeHistogram(self):
        ...
        print(' hist bin ,', self._hist_bins);
        ...
        # self._hist_bins = np.zeros((num_bins), dtype=np.float32)
        # index = 0
        # for value in hist_bins:
        #     print(value);
        #     if(value != 0.0):
        #         self._hist_bins[index] = math.log(value)
        #     else:
        #         self._hist_bins[index] = value

            # index += 1
        # print('hist ', self._hist_bins);
...
class Hist1DPlot:
    ...
    def plot(self, ax):
        ...
        # index = 0
        # log_color_density = []
        # for value in color_densities:
        #     print(value);
        #     if(value != 0.0):
        #         log_color_density.append(math.log(value))
        #     else:
        #         log_color_density.append(value)

        # min_logcolor = np.min(log_color_density);
        # max_logcolor = np.max(log_color_density);

        # log_color_density = (log_color_density - min_logcolor) / (max_logcolor - min_logcolor)
        
        # ax.bar(color_samples, log_color_density, width=width, color=colors, edgecolor='black')
        ax.bar(color_samples, color_densities, width=width, color=colors, edgecolor='black')
        print('color_samples ', color_samples);
        print('color_densities ', color_densities);
        print('color ', colors);      
