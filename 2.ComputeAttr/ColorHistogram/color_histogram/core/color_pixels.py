...
class ColorPixels:
    ...
    def _image2pixels(self, image):
        if _isGray(image):
            ...
            # return image.reshape((h * w))[self._mask]

        # image_array = image.reshape(-1, 3)
        # image_mask_array = self._mask.reshape(-1)
        # image_array = image_array[image_mask_array]

        h, w, cs = image.shape
        # print('origin ', image_array[0:4])
        # # image_mask = image[self._mask]
        # # h, w, cs = image_mask.shape;
        # # step = int(h * w / self._num_pixels)
        # return image_array[::2]
        # #remove the background color
        return image.reshape((-1, cs))[::2]
